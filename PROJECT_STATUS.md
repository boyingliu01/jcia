...
- **Adapters 覆盖率大幅补齐**：72.4% → 78.0% → **93.4%**（本轮补 4 个工具适配器单测：`skywalking_call_chain` 33%→100%、`java_all_call_graph` 61%→100%、`maven_surefire_test_executor` 61%→100%、`openai_adapter` 63%→99%）
- 质量门禁全面恢复：ruff 清零、pyright strict 0 错误、bandit 无告警、全量套件 **991 passed / 31 skipped**
- 整体测试覆盖率 **93%**（达标 ≥ 80%）
...
| Entities | ≥ 95% | **97.8%** | ✓ 达标 |
| Services | ≥ 85% | **90.1%** | ✓ 达标 |
| Adapters | ≥ 75% | **93.4%** | ✓ 达标（72.4% → 78.0% → 93.4%） |
| Use Cases | — | **98.4%** | ✓ |
| Infrastructure | — | **87.7%** | ✓ |
| Reports | — | **90.3%** | ✓ |
...
- Adapters 层累计从 72.4% 提升至 **93.4%**：在前一轮补齐 `SkyWalkingAdapter`（13%→100%）基础上，本轮进一步为 4 个依赖外部 shell/HTTP 的工具适配器补足单测——`skywalking_call_chain_adapter`（33%→100%）、`java_all_call_graph_adapter`（61%→100%，mock `subprocess.run`/`urllib.request.urlopen` 隔离 Java 调用与 JAR 下载边界）、`maven_surefire_test_executor`（61%→100%，隔离 Maven 边界）、`openai_adapter`（63%→99%，`sys.modules` 注入 fake 模块覆盖客户端与重试路径，仅 1 行不可达死代码）。至此该层此前记录的"外部工具适配器短板"已基本消除。
...
| 测试套件（全量） | 全通过 | **991 passed, 31 skipped** | ✓ |
...
| # | 问题 | 影响 | 状态 / 说明 |
|---|------|------|-------------|
| 1 | ~~部分外部工具适配器覆盖率偏低~~ | — | ✅ 已解决：`skywalking_call_chain`(33%→100%)、`java_all_call_graph`(61%→100%)、`maven_surefire_test_executor`(61%→100%)、`openai_adapter`(63%→99%) 已补足单测；Adapters 层整体 **93.4%** |
...
- **Adapters 覆盖率续补（本轮）**：为 4 个依赖外部 shell/HTTP 的工具适配器补足单测——`skywalking_call_chain_adapter`（33%→100%）、`java_all_call_graph_adapter`（61%→100%，隔离 `subprocess.run`/`urllib.request.urlopen`）、`maven_surefire_test_executor`（61%→100%，隔离 Maven）、`openai_adapter`（63%→99%，`sys.modules` 注入 fake 模块）；Adapters 层 78.0% → **93.4%**，整体 84% → **93%**，全量套件 895 → **991 passed**。
...
| ~~Adapters 覆盖率不达标~~ | — | — | ✅ 已解决 | 补 SkyWalkingAdapter 及 4 个工具适配器单测，72.4% → 78.0% → 93.4%（达标 ≥75%） |
...
3. ~~**补充 Adapters 覆盖率**~~ ✅ **已完成**：补 SkyWalkingAdapter（`f732d7a`）+ 4 个工具适配器单测，Adapters 层 72.4% → 78.0% → **93.4%**。
...
| 2026-09-14 | 2.2 | sqlite 两层同名去重（`2d2277c`）；补 4 个工具适配器单测使 Adapters 层 78.0% → 93.4%、整体 84% → 93%、全量 895 → 991 passed；同步刷新本报告与 CLAUDE/AGENTS 覆盖率数字 | Qoder |
