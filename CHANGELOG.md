...
### Added
- 资源守卫改为从 pyproject `[tool.setuptools.package-data]` 自动派生（消除守卫与打包配置的双源维护，派生结果经 `GuardConfig` 注入 `check()`）（#23-REQ-01）
- 守卫新增 sdist 正向资源断言（前缀候选集匹配 + 前导 `./` 容错）与「wheel 内源码树不存在的包内 `.py`」单向反查（陈旧构建缓存向量）（#23-REQ-02/REQ-06）
- 守卫在拼接源码路径前拒绝路径不安全的 wheel 成员名（绝对路径、`..` 段、反斜杠）（#23-REQ-02）
- CONTRIBUTING PR 检查清单新增公开 API 变更记录项（#23-REQ-08）
- CI 新增 `packaging` 独立阻塞 job：常规 PR 流程构建 wheel+sdist 并跑资源守卫，打包回归在 PR 阶段转红，而不是拖到发版才暴露（该链路此前只在 tag push 时首跑）（#23-REQ-02）

### Changed
- 守卫要求 `dist/` 同时包含 wheel 与 sdist：缺任一产物均 fail-closed（缺 sdist 的错误消息含 `python -m build` 修复提示）。显式前提：守卫假定发布链路以 `python -m build` 全量构建，solo-wheel 本地构建将失败（#23-REQ-06，D6）
- **公开 API 行为变更**：修复 `find_test_classes` 对空字符串输入（`""`、`"com.example."`）返回虚假匹配的问题（现返回 `[]`；该方法当前无生产调用方）（#23-REQ-07）
...
- 消除占位邮箱外泄：README「支持」章节的 `jcia-dev@example[.]org` 经 `readme = "README.md"` 进入 `METADATA` 的 `long_description`，发布后会永久渲染在 PyPI 项目页且不可编辑（yank 亦保留历史），故删除该行、支持渠道只保留 GitHub Issues；同时删除 `jcia/__init__.py` 中全仓零引用的 `__email__ = "jcia@example[.]com"`（实测已随 wheel 与 sdist 分发）。本条目内的占位域名一律以 `[.]` 记法书写，避免正文本身成为守卫的匹配源
- `release.yml` 加固：`build` 在 `twine check` 后调用新增的 `scripts/check_dist_placeholders.py`，对 `dist/` 下**每一个文件**（而非按后缀筛出的子集）做封闭分类并解开全部成员逐行扫描占位域名；无法识别的产物走尽力扫描并**追加一条阻断结论**——读不懂的发布物绝不默认放行（`twine check` 只校验元数据格式、pre-push 的 Gate MW 只比对 commit diff，二者结构上都抓不到这类缺陷）；`publish-pypi` 显式 `skip-existing: "true"`——上游默认为 `false`，一旦发布成功后误点 Re-run all jobs 必返 409，而 `github-release` 因 `needs` 不满足永不执行，会形成「PyPI 已发布、GitHub Release 缺失且 UI 无恢复路径」的终态
